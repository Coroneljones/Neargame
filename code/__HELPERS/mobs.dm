...
/proc/ageAndGender2Desc(age, gender)//Used for the radio
	if(gender == FEMALE)
		switch(age)
			...
			if(18 to 25)		return "Young Woman"
			if(25 to 60)		return "Woman"
			if(60 to 120)	return "Old Woman"
			else				return "Unknown"
	else
		switch(age)
			if(0 to 18)			return "Boy"
			if(18 to 25)		return "Young Man"
			if(25 to 60)		return "Man"
			if(60 to 120)	return "Old Man"
			else				return "Unknown"

...
/obj/machinery/computer/forensic_scanning/attack_hand(mob/user)
	...
	if(!authenticated)
		...
		dat += "<a href='byond://?src=\ref[src];operation=logout'>{Log Out}</a>"
		dat += " | "
		dat += "<a href='byond://?src=\ref[src];operation=screen;screen=database'>Database</a>"
		dat += " | "
		dat += "<a href='byond://?src=\ref[src];operation=screen;screen=details'>Record details</a>"
		dat += " | "
		dat += "<a href='byond://?src=\ref[src];operation=screen;screen=scan'>Scanning</a>"
		dat +="<br><hr><br>"
		switch(screen)
			if("database")	//Database screen
				...
				for(var/filter in filternames)
					...
					dat += "<br>[filter]: <a href='byond://?src=\ref[src];operation=filter;filter=[fname]'>[filters[fname] ? list2text(filters[fname], ",") : "All"]</a>"
				...
				if(current_list.len < 1)
					...
					for(var/datum/data/record/forensic/record in current_list)
						dat += "<tr><td><a href='byond://?src=\ref[src];operation=details;identifier=[record.uid]'>[record.fields["name"]]</td>"
						...
						dat += "<td><a href='byond://?src=\ref[src];operation=delete;identifier=[record.uid]'>Delete</a></td>"
						dat += "</tr>"
					dat += "</table>"
					dat += "<a href='byond://?src=\ref[src];operation=printall'>Print all listed</a><br>"
			...
			if("details")	//Details screen
				if(!current)
					...
					dat += "<b>Labels:</b> "
					dat += "<a href='byond://?src=\ref[src];operation=label'>[current.fields["label"] ? current.fields["label"] : "None"]</a><br>"
					dat += "<a href='byond://?src=\ref[src];operation=print'>Print record</a><br>"

			if("scan")	//Scanning screen
				dat += "Object: <a href='byond://?src=\ref[src];operation=object'>[scanning ? scanning.name : "-----"]</a><br>"
				if (scanning)
					if (scan_progress > 0)
						dat += "Scan in progress."
						dat += " <a href='byond://?src=\ref[src];operation=cancel'>Cancel</a><br>"
					else
						dat += "<a href='byond://?src=\ref[src];operation=scan'>Scan</a><br>"
				dat += "Insert fingerprint card here: <a href='byond://?src=\ref[src];operation=card'>-----</a>"

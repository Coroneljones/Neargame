...
var/static/list/family_blacklisted_jobs = list("Bulter", "Sitzfrau", "Charybdis", "Praetor", "Sniffer", "Guest")
var/global/datum/matchmaker/matchmaker = new()
var/datum/family/baron_family = null
var/list/mob/living/carbon/human/pending_nobles = list()
var/list/mob/living/carbon/human/pending_family = list()
var/list/datum/set_spouse/pending_set_spouse = list() //holds pending set_spouses
var/list/client/set_spoused = list() //prevents set_spoused people from being shoved into other families
...
/datum/set_spouse/proc/spouse_check()
	...
	if(ticker.eof.id == "ordinators" && (human_target.job == "Kraken" || human_target.job == "Triton" || human_sender.job == "Triton" || human_sender.job == "Kraken"))
		return 0
...
/datum/matchmaker/proc/do_family_matchmaking()
	...
	for(var/mob/living/carbon/human/H in mob_list)
		...
		if(ticker.eof.id == "ordinators" && (H.job == "Kraken" || H.job == "Triton")) continue
		if(baronfamily.Find(H.job))
			...
			if(LAZYLEN(families) < total_familes) // If we aren't at our limit yet, we make five suitable players head of house
	...
	for(var/mob/living/carbon/human/M in pending_family)
		var/list/datum/family/families_list = families - baron_family //as funny as it is a random bum being in the baron's family is stupid
		var/datum/family/chosen_family
		if(LAZYLEN(families_list))
...
/datum/matchmaker/proc/setmarriage(mob/living/carbon/human/owner, mob/living/carbon/human/target)
...
/mob/living/carbon/human/proc/adjust_age(target_age, direction)
	switch(direction)
		if(AGE_SAME)

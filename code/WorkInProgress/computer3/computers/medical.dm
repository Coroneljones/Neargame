...
/datum/file/program/med_data
	...
	interact()
		...
		if (temp)
			...
			dat = text("Confirm Identity (R): <A href='byond://?src=\ref[];cardr=1'>[]</A><HR>", src, (scan ? text("[]", scan.name) : "----------"))
			if (computer.cardslot.dualslot)
				dat += text("Check Identity (W): <A href='byond://?src=\ref[];cardw=1'>[]</A><BR>", src, (scan2 ? text("[]", scan2.name) : "----------"))
				if(scan2 && !scan)

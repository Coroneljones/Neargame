	...
	proc/main_menu()
		...
		if (computer.radio.subspace)
			if(emergency_shuttle.online() && emergency_shuttle.location())
				...
				refresh = 0
		if (authenticated)
			dat += "<BR>\[ <A HREF='byond://?src=\ref[src];logout'>Log Out</A> \]"
			if (authenticated==2)
				dat += "<BR>\[ <A HREF='byond://?src=\ref[src];announce'>Make An Announcement</A> \]"
				if(computer.emagged == 0)
					dat += "<BR>\[ <A HREF='byond://?src=\ref[src];MessageCentcomm'>Send an emergency message to Centcomm</A> \]"
				else
					dat += "<BR>\[ <A HREF='byond://?src=\ref[src];MessageSyndicate'>Send an emergency message to \[UNKNOWN\]</A> \]"
					dat += "<BR>\[ <A HREF='byond://?src=\ref[src];RestoreBackup'>Restore Backup Routing Data</A> \]"

				dat += "<BR>\[ <A HREF='byond://?src=\ref[src];changeseclevel'>Change alert level</A> \]"
			if(emergency_shuttle.location())
				if (emergency_shuttle.online())
					dat += "<BR>\[ <A HREF='byond://?src=\ref[src];cancelshuttle'>Cancel Shuttle Call</A> \]"
				else
					dat += "<BR>\[ <A HREF='byond://?src=\ref[src];callshuttle'>Call Emergency Shuttle</A> \]"

			dat += "<BR>\[ <A HREF='byond://?src=\ref[src];status'>Set Status Display</A> \]"
		else
			dat += "<BR>\[ <A HREF='byond://?src=\ref[src];login'>Log In</A> \]"
		dat += "<BR>\[ <A HREF='byond://?src=\ref[src];messagelist'>Message List</A> \]"
		return dat
		...
		switch(state)
			...
			if(STATE_MESSAGELIST)
				...
				for(var/i = 1; i<=messagetitle.len; i++)
					dat += "<BR><A HREF='byond://?src=\ref[src];viewmessage;message-num=[i]'>[messagetitle[i]]</A>"
			if(STATE_VIEWMESSAGE)
				if (currmsg)
					dat += "<B>[messagetitle[currmsg]]</B><BR><BR>[messagetext[currmsg]]"
					if (authenticated)
						dat += "<BR><BR>\[ <A HREF='byond://?src=\ref[src];delmessage'>Delete \]"
				else
					state = STATE_MESSAGELIST
					interact()
					return
			if(STATE_DELMESSAGE)
				if (currmsg)
					dat += "Are you sure you want to delete this message? \[ <A HREF='byond://?src=\ref[src];delmessage2'>OK</A> | <A HREF='byond://?src=\ref[src];viewmessage'>Cancel</A> \]"
				else
					state = STATE_MESSAGELIST
					interact()
					return
			if(STATE_STATUSDISPLAY)
				dat += "\[ <A HREF='byond://?src=\ref[src];main'>Back</A> \]<BR>"
				dat += "Set Status Displays<BR>"
				dat += "\[ <A HREF='byond://?src=\ref[src];setstat;statdisp=blank'>Clear</A> \]<BR>"
				dat += "\[ <A HREF='byond://?src=\ref[src];setstat;statdisp=time'>Station Time</A> \]"
				dat += "\[ <A HREF='byond://?src=\ref[src];setstat;statdisp=shuttle'>Shuttle ETA</A> \]<BR>"
				dat += "\[ <A HREF='byond://?src=\ref[src];setstat;statdisp=message'>Message</A> \]"
				dat += "<ul><li> Line 1: <A HREF='byond://?src=\ref[src];setmsg1'>[ stat_msg1 ? stat_msg1 : "(none)"]</A>"
				dat += "<li> Line 2: <A HREF='byond://?src=\ref[src];setmsg2'>[ stat_msg2 ? stat_msg2 : "(none)"]</A></ul><br>"
				dat += "\[ Alert: <A HREF='byond://?src=\ref[src];setstat;statdisp=alert;alert=default'>None</A> |"
				dat += " <A HREF='byond://?src=\ref[src];setstat;statdisp=alert;alert=redalert'>Red Alert</A> |"
				dat += " <A HREF='byond://?src=\ref[src];setstat;statdisp=alert;alert=lockdown'>Lockdown</A> |"
				dat += " <A HREF='byond://?src=\ref[src];setstat;statdisp=alert;alert=biohazard'>Biohazard</A> \]<BR><HR>"
			if(STATE_ALERT_LEVEL)
				dat += "Current alert level: [get_security_level()]<BR>"
				if(security_level == SEC_LEVEL_DELTA)
					dat += "<font color='red'><b>The self-destruct mechanism is active. Find a way to deactivate the mechanism to lower the alert level or evacuate.</b></font>"
				else
					dat += "<A HREF='byond://?src=\ref[src];securitylevel;newalertlevel=[SEC_LEVEL_BLUE]'>Blue</A><BR>"
					dat += "<A HREF='byond://?src=\ref[src];securitylevel;newalertlevel=[SEC_LEVEL_GREEN]'>Green</A>"
			if(STATE_CONFIRM_LEVEL)
				dat += "Current alert level: [get_security_level()]<BR>"
				dat += "Confirm the change to: [num2seclevel(tmp_alertlevel)]<BR>"
				dat += "<A HREF='byond://?src=\ref[src];swipeidseclevel'>Swipe ID</A> to confirm change.<BR>"

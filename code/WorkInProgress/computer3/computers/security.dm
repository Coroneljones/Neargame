...
/datum/file/program/secure_data
	...
	interact()
		...
		if (temp)
			...
			dat = text("Confirm Identity (R): <A href='byond://?src=\ref[];choice=Confirm Identity R'>[]</A><HR>", src, (scan ? text("[]", scan.name) : "----------"))
			if (computer.cardslot.dualslot)
				dat += text("Check Identity (W): <A href='byond://?src=\ref[];choice=Confirm Identity W'>[]</A><BR>", src, (scan2 ? text("[]", scan2.name) : "----------"))
				if(scan2 && !scan)
